...
public final class HtmlNormalizer {
  ...
  /**
   * Sanitizes the HTML to keep only semantic text structures and elements, while removing styles,
   * classes, scripts, header/footer/nav layout elements. Also converts hyperlinks to inline text
   * format so URL values are preserved.
   */
  ...
  public static String sanitizeHtml(String rawHtml, String baseUrl) {
    ...
    doc.select("script, style, iframe, noscript, svg, picture, header, footer, nav, aside, form")
        .remove();
    ...
    return baseUrl != null
        ? Jsoup.clean(doc.body().html(), baseUrl, safelist)
        : Jsoup.clean(doc.body().html(), safelist);
  }
